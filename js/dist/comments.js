  ...
  var Comments = {
    ...
    relay: function relay() {
      $(document).on('click', 'a.comments-replay', function (event) {
        ...
        temp = temp.replace(new RegExp('{{img}}', 'g'), subdata.imgurl || '/img/user.png').replace(new RegExp('{{website}}', 'g'), subdata.website || 'javascript:void(0)').replace(new RegExp('{{author}}', 'g'), subdata.author || '外星人');
    ...
    InitComments: function InitComments(base) {
      ...
      for (var i in list) {
        ...
        temp = temp.replace(new RegExp('{{author}}', 'g'), item.author).replace(new RegExp('{{img}}', 'g'), item.imgurl || '/img/user.png').replace(new RegExp('{{website}}', 'g'), item.website || 'javascript:void(0)').replace(new RegExp('{{active}}', 'g'), itemActive).replace(new RegExp('{{love}}', 'g'), item.love).replace(new RegExp('{{comment}}', 'g'), Comments.contentHandle(item.comment)).replace(new RegExp('{{ctime}}', 'g'), ctime);
        var $temp = $(temp);
        ...
        if (replys) {
          for (var z in replys) {
            ...
            replayTemp = replayTemp.replace(new RegExp('{{author}}', 'g'), reply.author).replace(new RegExp('{{img}}', 'g'), reply.imgurl || '/img/user.png').replace(new RegExp('{{website}}', 'g'), reply.website || 'javascript:void(0)').replace(new RegExp('{{active}}', 'g'), replayActive).replace(new RegExp('{{love}}', 'g'), reply.love).replace(new RegExp('{{comment}}', 'g'), Comments.contentHandle(reply.comment)).replace(new RegExp('{{ctime}}', 'g'), replyTime);

            if (reply.topid !== '0' && reply.topid !== reply.superid) {
              superTemp = superTemp.replace(new RegExp('{{superauthor}}', 'g'), reply.superauthor || '外星人').replace(new RegExp('{{superimg}}', 'g'), reply.superimgurl || '/img/user.png').replace(new RegExp('{{superwebsite}}', 'g'), reply.superwebsite || 'javascript:void(0)');
            } else {
              superTemp = '';
            }
